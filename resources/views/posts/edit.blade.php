<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        <form action="{{ route('posts.update', $post->id) }}" method="POST">
            ...
            <div class="mb-4">
                <label for="title" class="block font-medium text-sm text-gray-700">Naslov</label>
                <input type="text" name="title" id="title" value="{{ $post->title }}" class="w-full border rounded px-3 py-2" required>
            </div>

            <div class="mb-4">
                <label for="content" class="block font-medium text-sm text-gray-700">Sadržaj</label>
